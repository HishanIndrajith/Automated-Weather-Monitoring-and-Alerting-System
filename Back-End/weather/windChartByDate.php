...
$mysql_qry = "SELECT date,wind_speed FROM data_set1 WHERE date >= '".$date_selected." 00:00:00' AND date <= '".$date_selected." 23:59:59'  AND location = ". $location." ORDER BY date ASC ";
$result = mysqli_query($conn,$mysql_qry);
$myJSON="";
if(mysqli_num_rows($result) > 0){
    $ret = array();
    $no=1;
    $ret[0]=["time","wind speed (kmph)"];//headers
    while($row = $result->fetch_assoc()){
        $arr=array();
        $pieces = explode(" ", $row["date"]);
        $arr[0] = $pieces[1];
        $arr[1] = intval($row["wind_speed"]);
        $ret[$no] = $arr;
